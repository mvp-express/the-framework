...
This comprehensive guide explains how to use the MVP.Express codec module for high-performance, zero-copy message encoding and decoding using the MYRA (Memory Yield, Rapid Access) codec.
...
registry.registerMessage(1, GetBalanceRequest.class);
registry.registerMessage(2, GetBalanceResponse.class);
...
envelope.setMethodId((short) 1);
envelope.setCorrelationId(12345L);
envelope.setFlags((byte) 0);
...
// Monitor pool usage
System.out.println("Available segments: " + pool.getAvailableCount());
System.out.println("Total segments: " + pool.getTotalCount());
```
...
System.out.println("Cache size: " + codec.getCacheSize());
...
// Set headers
envelope.setMethodId((short) 42);
envelope.setCorrelationId(System.nanoTime());
envelope.setFlags(MessageEnvelope.FLAG_IS_RESPONSE);
...
// ... use envelope
envelope.release(); // Return to pool
...
try {
    envelope = MessageEnvelope.allocate(256, pool);
    codec.encodeMessage(message, envelope);
} finally {
    if (envelope != null) {
        envelope.release();
...
public record UserInfo(String name, String email) {} // OK

// ⚠️ CAUTION: Very long strings may impact performance
public record LargeText(String content) {} // Consider byte[] for large data
```
...
if (pool.getAvailableCount() == 0) {
    System.err.println("Pool exhausted! Check for unreleased envelopes");
}
```
...
System.out.println("Encode time: " + encodeTime + "ns");
System.out.println("Cache size: " + codec.getCacheSize());
System.out.println("Pool available: " + pool.getAvailableCount());
```
...
try {
    ...
    networkChannel.write(envelope.getBuffer());
} finally {
    envelope.release();
}
...
For more advanced usage and integration patterns, see the [Codegen Usage Guide](codegen-usage-guide.md) and [Transport Integration Guide](transport-integration-guide.md).